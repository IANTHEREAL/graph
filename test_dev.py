...
from graph_opt.optimizer import process_entity_quality_issue, process_redundancy_entity_issue, process_relationship_quality_issue, process_redundancy_relationship_issue
from llm.embedding import get_text_embedding, get_entity_description_embedding, get_entity_metadata_embedding
...
def improve_graph(query: str, tmp_test_data_file: str = "test_data.pkl"):
    if os.path.exists(tmp_test_data_file):
        ...
        prompt = GRAPH_OPTIMIZATION_ACTION_SYSTEM_PROMPT_WO_MR + " Now Optimize the following graph:\n" + json.dumps(graph_data, indent=2, ensure_ascii=False)
    ...
    for index, row in issue_df.iterrows():
        ...
        if row['issue']['issue_type'] == 'entity_quality_issue' or row['issue']['issue_type'] == 'relationship_quality_issue':
    ...
    for index, row in issue_df.iterrows():
        if row['issue']['issue_type'] != 'entity_quality_issue' or row['confidence'] < 1.8 or row['resolved'] is True:
            continue
        ...
        for affected_id in row['issue']['affected_ids']:
            ...
            if issue_cache.get(issue_key, False) or pending_entity_quality_issue_list.get(issue_key, None) is not None:
                continue
            issue['issue_key'] = issue_key
            pending_entity_quality_issue_list[issue_key] = issue


    print("pendding entity quality issues number", len(pending_entity_quality_issue_list))
    ...
    while True:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(batch_issues)) as executor:
            futures = {}
            for row_issue in batch_issues:
                futures[executor.submit(process_entity_quality_issue, sonnet_critic_client, Entity, Relationship, row_issue['row_index'], row_issue)] = row_issue['issue_key']
    ...
    for index, row in issue_df.iterrows():
        if row['issue']['issue_type'] != 'redundancy_entity' or row['confidence'] < 1.8 or row['resolved'] is True:
            continue
        ...
        while found:
            ...
            for other_row_index, other_row in issue_df.iterrows():
                if other_row_index == index or other_row_index in handled_index:
                    continue
                if other_row['issue']['issue_type'] != 'redundancy_entity' or other_row['confidence'] < 1.8 or other_row['resolved'] is True:
                    continue
    ...
    print("pendding redundancy entity number", len(pending_redundancy_entity_issue_list))
    ...
    while True:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(batch_issues)) as executor:
            ...
            for row_issue in batch_issues.values():
                futures[executor.submit(process_redundancy_entity_issue, sonnet_critic_client, Entity, Relationship, row_issue['issue_key'], row_issue)] = row_issue['issue_key']
    ...
    for index, row in issue_df.iterrows():
        if row['issue']['issue_type'] != 'relationship_quality_issue' or row['confidence'] < 1.8 or row['resolved'] is True:
            continue
        ...
        for affected_id in row['issue']['affected_ids']:
            ...
            issue_key = get_issue_key(issue)
            if issue_cache.get(issue_key, False) or pending_relationship_quality_issue_list.get(issue_key, None) is not None:
                continue
            issue['issue_key'] = issue_key
            pending_relationship_quality_issue_list[issue_key] = issue

    print("pendding relationship quality issues number", len(pending_relationship_quality_issue_list))
    ...
    while True:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(batch_issues)) as executor:
            futures = {}
            for row_issue in batch_issues:
                futures[executor.submit(process_relationship_quality_issue, sonnet_critic_client, Relationship, row_issue['row_index'], row_issue)] = row_issue['issue_key']
    ...
    for index, row in issue_df.iterrows():
        if row['issue']['issue_type'] != 'redundancy_relationship' or row['confidence'] < 1.8 or row['resolved'] is True:
            continue
        ...
        while found:
            ...
            for other_row_index, other_row in issue_df.iterrows():
                if other_row_index == index or other_row_index in handled_index:
                    continue
                if other_row['issue']['issue_type'] != 'redundancy_relationship' or other_row['confidence'] < 1.8 or other_row['resolved'] is True:
                    continue
                other_affected_ids = set(other_row['issue']['affected_ids'])
                if need_merge_ids.isdisjoint(other_affected_ids):
                    continue
        ...
        if len(need_merge_ids) > 1:
            ...
            pending_redundancy_relationships_issue_list[issue_key] = redundancy_relationship_issue

    print("pendding redundancy relationships number", len(pending_redundancy_relationships_issue_list))
    ...
    while True:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(batch_issues)) as executor:
            futures = {}
            for row_issue in batch_issues.values():
                futures[executor.submit(process_redundancy_relationship_issue, sonnet_critic_client, Relationship, row_issue['issue_key'], row_issue)] = row_issue['issue_key']
